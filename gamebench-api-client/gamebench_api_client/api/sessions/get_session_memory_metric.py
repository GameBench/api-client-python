...
def _get_kwargs(
    ...
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/v2/sessions/{session_id}/metrics/memory".format(session_id=quote(str(session_id), safe=""),),
    }
...
def sync_detailed(
    ...
    *,
    ...
    caller's data scope. Both checks are enforced router-wide
    across `/v2/sessions/{sessionId}/metrics/*` — every endpoint
    under this prefix returns 404 when the session does not
    exist OR the caller's data scope hides it.

    Response shape is the raw output of the metric finder and
    depends on the recording's sample stream. Treat it as an
    open object — fields and units vary by metric class.
